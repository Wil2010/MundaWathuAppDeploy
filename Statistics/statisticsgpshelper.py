...
class StatisticsGpsHelper():
    has_centered_map = False
    dialog = None

    def run(self):
        # Get a reference to GpsBlinker, then call blink()
        statistics_gps_blinker = App.get_running_app().statisticsmapview.ids.statisticsblinker

        # Start blinking the GpsBlinker
        statistics_gps_blinker.blink()
    ...
    def update_blinker_position(self, *args, **kwargs):
        ...
        statistics_gps_blinker = App.get_running_app().statisticsmapview.ids.occupationblinker
        statistics_gps_blinker.lat = my_lat
        statistics_gps_blinker.lon = my_lon

        # Center map on gps
        if not self.has_centered_map:
            statistics_map = App.get_running_app().statisticsmapview
            statistics_map.center_on(my_lat, my_lon)
